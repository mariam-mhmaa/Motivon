#!/usr/bin/env python3
"""Deploy v23 navigator: WP before T3, safe WP after T5, anti-vibration homing."""
import os, stat
...
Route: T1 -> T2 -> WP0 -> T3 -> T4 -> WP1 -> WP2 -> T5 -> WP3 -> WP4 -> HOME

v23: Added WP0 (-0.86, 1.53) before T3.
     Added WP3a (0.07, -0.99) after T5 for safe clearance.
     Renamed old WP3 -> WP4.
     Anti-vibration homing: larger home tolerance, velocity ramp-down,
     wider yaw tolerance for final alignment.  Faces forward-left (spawn yaw).
"""
...
class RobotNavigatorNode(Node):
    ...
    SPAWN_X = 2.5
    SPAWN_Y = -1.46
    SPAWN_YAW = math.pi / 2.0   # facing +Y = forward-left

    WORLD_TARGETS = [
        (2.5,   0.5),      # 0: T1
        (0.04,  1.01),     # 1: T2
        (-0.86, 1.53),     # 2: WP0  (new waypoint before T3)
        (-1.75, 1.45),     # 3: T3
        (-2.76, 0.65),     # 4: T4
        (-2.76, 0.22),     # 5: WP1  (forward, same X as T4)
        (-1.45, 0.22),     # 6: WP2  (strafe east)
        (-0.55, -0.39),    # 7: T5
        (0.07, -0.99),     # 8: WP3  (safe clearance after T5)
        (1.50, -1.20),     # 9: WP4  (approach home safely)
    ]
    LID_INDICES = frozenset({0, 1, 3, 4, 7})    # delivery targets with lid
    FORCED_INDICES = frozenset({2, 5, 6, 8, 9})  # waypoints: no sensor gating
    TARGET_NAMES = ['T1','T2','WP0','T3','T4','WP1','WP2','T5','WP3','WP4']

    VERSION = 'v23'
    GOAL_TOLERANCE = 0.10
    HOME_TOLERANCE = 0.25        # wider tolerance to prevent vibration at home
    ...
    T4_IDX = 4
    WALL_APPROACH_INDICES = frozenset({4})  # T4 near west wall
    ...
    KP_ROT = 0.5
    MAX_TURN = 0.3
    YAW_TOLERANCE = 0.12        # ~7 deg, wider to prevent oscillation
    ...
    def __init__(self):
        ...
        self._stop_count = 0       # count consecutive stop commands for clean halt
    ...
    def _do_navigate(self):
        ...
        # Choose tolerance based on target type
        if self.state == self.STATE_RETURNING:
            tol = self.HOME_TOLERANCE
        elif self.target_idx in self.WALL_APPROACH_INDICES:
            tol = self.WALL_APPROACH_TOL
        elif self.target_idx not in self.LID_INDICES:
            tol = self.WAYPOINT_TOL
        else:
        ...
        if dist < tol:
            ...
            self._wall_override = False
            if self.state == self.STATE_RETURNING:
                # Send several zero-vel commands to let physics settle
                self._stop_count += 1
                if self._stop_count < 10:
                    return
                self._stop_count = 0
                self.state = self.STATE_ALIGNING
                self.get_logger().info(
                    'Home reached. Aligning to initial orientation (forward-left)...')
        ...
        self._stop_count = 0  # reset if we move again
        ...
        # Slow down near any goal
        if dist < tol * 3:
            vx_raw *= 0.5
            vy_raw *= 0.5

        # Extra slow-down for homing to prevent overshoot
        if self.state == self.STATE_RETURNING and dist < self.HOME_TOLERANCE * 2:
            vx_raw *= 0.4
            vy_raw *= 0.4
        ...
        if (self.state == self.STATE_NAVIGATING
            ...
            if mag > 0.01:
                ...
                # Ramp down speed when close to home
                if self.state == self.STATE_RETURNING and dist < 0.5:
                    speed *= (dist / 0.5)
    ...
    def _do_align(self):
        """Rotate to SPAWN_YAW (pi/2 = facing forward-left / +Y)."""
        yaw_error = self._normalize(self.SPAWN_YAW - self.robot_yaw)
        if abs(yaw_error) < self.YAW_TOLERANCE:
            self._stop()
            self.state = self.STATE_IDLE
            self.get_logger().info(
                f'Orientation aligned (yaw={math.degrees(self.robot_yaw):.1f} deg, '
                f'facing forward-left). IDLE.')
            return
        # Proportional with velocity ramp-down near target to prevent oscillation
        raw_w = self.KP_ROT * yaw_error
        # Reduce gain when error is small to avoid overshoot/vibration
        if abs(yaw_error) < 0.3:
            raw_w *= 0.5
        cmd = Twist()
        cmd.angular.z = -self._clamp(raw_w, -self.MAX_TURN, self.MAX_TURN)
        if self.log_tick % 20 == 0:
            self.get_logger().info(
...
print(f'Written {len(CONTENT)} bytes to {dst} -> v23')
