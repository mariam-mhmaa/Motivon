...
class RobotNavigatorNode(Node):
    ...
    # SPAWN_YAW of pi/2 in world = 0 in local.
    ...
    # Per-target yaw in LOCAL odom frame.
    # Applied after lid-cycle at delivery targets, or on arrival at waypoints.
    # Targets not listed here receive no rotation.
    #   local yaw = world yaw - SPAWN_YAW (pi/2)
    #   South (world -pi/2) → local pi
    #   East  (world  0)    → local -pi/2
    #   North (world  pi/2) → local 0
    #   West  (world  pi)   → local pi/2   (= IDLE_YAW)
    TARGET_YAWS = {
        0:  math.pi,            # T1  → South ↓
        1:  math.pi,            # T2  → South ↓
        3: -math.pi / 2.0,      # T3  → East  →
        5:  0.0,                # WP1 → North ↑
    }
    ...
    def _do_lid_close(self):
        ...
        if self._call_lid(False):
            ...
            if self.target_idx in self.TARGET_YAWS:
                self.rotate_target_yaw = self.TARGET_YAWS[self.target_idx]
                self.state = self.STATE_ROTATING
                self.get_logger().info(
                    f'Rotating to {math.degrees(self.rotate_target_yaw):.0f} deg (local odom).')
            else:
    ...
    def _do_align(self):
        """Rotate in place to IDLE_YAW (pi/2 in local odom = West on screen)."""
        yaw_error = self._normalize(self.IDLE_YAW - self.robot_yaw)
        if abs(yaw_error) < self.YAW_TOLERANCE:
            self._stop()
        ...
        if self.log_tick % 20 == 0:
            self.get_logger().info(
                ...
                f'target={math.degrees(self.IDLE_YAW):.1f} deg  '
                f'err={math.degrees(yaw_error):.1f} deg  '
                f'w={cmd.angular.z:.2f}')
        self.cmd_pub.publish(cmd)
    ...
    def _do_navigate(self):
            ...
            if self.state == self.STATE_RETURNING:
                ...
                if self.target_idx in self.TARGET_YAWS:
                    ...
                    self.get_logger().info(
                        ...
                        f'Rotating to {math.degrees(self.rotate_target_yaw):.0f} deg.')
                else:
                    self.get_logger().info(
                        f'{self.TARGET_NAMES[self.target_idx]} reached at '
        ...
        if ((self.state == self.STATE_NAVIGATING
            ...
            # Dynamic obstacle present: bypass sensor gate to prevent backup trigger.
            # KP_DRIVE proportional control decelerates to zero as dist → 0.
            vx = vx_raw
            vy = vy_raw
            phase = 'OBS'
        else:
