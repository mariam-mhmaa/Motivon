...
class TwoStationTest(Node):
    def __init__(self) -> None:
        super().__init__("two_station_navigation_test")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the supervised HOME -> WP1 -> WP2 test."
    )
    parser.add_argument("--hold-time", type=float, default=10.0)
    parser.add_argument("--target-timeout", type=float, default=120.0)
    parser.add_argument(
        "--area-clear",
        action="store_true",
...
def main(args=None) -> None:
    ...
    if not parsed.area_clear:
        raise SystemExit(
            "REFUSED: clear HOME -> WP1 -> WP2 and add --area-clear"
        )
    if (
