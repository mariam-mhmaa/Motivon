...
from motivon_interfaces.msg import ObstacleScan
...
class UltrasonicNode(Node):
    ...
    def __init__(self):
        ...
        self.scan_pub = self.create_publisher(
            ObstacleScan, "/obstacle/scan", sensor_qos
        )
        self.raw_pub = self.create_publisher(
            ObstacleScan, "/obstacle/raw_scan", sensor_qos
        )
    ...
    def scan_once(self):
        ...
        raw_msg = self.make_scan_message(stamp, front_label, back_label, raw)
        self.raw_pub.publish(raw_msg)

        scan_msg = self.make_scan_message(
            stamp, front_label, back_label, self.samples
        )
        self.scan_pub.publish(scan_msg)

        self.print_distances_if_due(front_label, back_label)
        if self.stationary_scan is not None:
            self.advance_stationary_scan(measured["front"])

    def make_scan_message(self, stamp: Time, front_label, back_label, values):
        msg = ObstacleScan()
        msg.stamp = stamp
        msg.active_front = str(front_label)
        msg.active_back = str(back_label)
        self.assign_distance(msg, "front_cm", "front_valid", values, "front_center")
        self.assign_distance(
            msg, "front_left_cm", "front_left_valid", values, "front_left"
        )
        self.assign_distance(
            msg, "front_right_cm", "front_right_valid", values, "front_right"
        )
        self.assign_distance(msg, "back_cm", "back_valid", values, "back_center")
        self.assign_distance(msg, "left_cm", "left_valid", values, "left")
        self.assign_distance(msg, "right_cm", "right_valid", values, "right")
        return msg

    @staticmethod
    def assign_distance(msg, value_field, valid_field, values, key):
        value = finite_distance(values.get(key))
        if value is None:
            setattr(msg, value_field, math.nan)
            setattr(msg, valid_field, False)
        else:
            setattr(msg, value_field, float(value))
            setattr(msg, valid_field, True)
    ...
    def print_distances_if_due(self, active_front, active_back):
